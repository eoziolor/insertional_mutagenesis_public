...
# Function that loads in data and creates combined objects
loadIS <- function(meta,
  ...
  if(do.vir.origin == TRUE){
    ...
    for(i in files){
      ...
      if(class(vir.ranges)=="GRanges"){
        ...
        test<-gsub(".*unaligned.*","mate",test.t)
  ...
  if(do.read.info == TRUE){
    ...
    files<-list.files(path = paste0(base.dir,"/reads_info"), pattern = "[0-9]+\\_reads\\_info.txt$", full.names = TRUE)
    ...
    for(i in files){
      ...
      host.all <- if(file.exists(paste0(base.dir,"reads_info/",nam1,"_reads_info.txt"))){
        fread(paste0(base.dir,"reads_info/",nam1,"_reads_info.txt"))
      ...
      virus.soft <- if(file.exists(paste0(base.dir,"reads_info/",nam1,"_softclipped_mapped_filtered_reads_info.txt"))){
        fread(paste0(base.dir,"reads_info/",nam1,"_softclipped_mapped_filtered_reads_info.txt"))
      }else{data.table(source = c(), type = c())}
      virus.mate <- if(file.exists(paste0(base.dir,"reads_info/",nam1,"_unaligned_mapped_filtered_reads_info.txt"))){
        fread(paste0(base.dir,"reads_info/",nam1,"_unaligned_mapped_filtered_reads_info.txt"))
      }else{data.table(source = c(), type = c())}
      ...
      if(!is.null(read.info) & dim(as.data.frame(read.info))[1]>0){
        colnames(read.info) <- c("read","chromosome","start","cigar","source","type")
